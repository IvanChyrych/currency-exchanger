...
const DevicePage = () => {
    const [device, setDevice] = useState({ info: [] });
    const [amount, setAmount] = useState(''); // Состояние для хранения количества
    const { id } = useParams();
    ...
    const add = () => {
        const formData = new FormData();
        formData.append('basket_id', '1');
        formData.append('device_id', '1');
        formData.append('amount', amount); // Добавляем количество
        formData.append('currency', '1'); // Добавляем id валюты, если нужно
        addToBasket(formData)
        .then(response => alert(`Товар ${device.name} был добавлен в вашу корзину!`))
            ...
            <Row>
                ...
                <Col md={4}>
                    ...
                    >
                        ...
                        <input
                            type="text"
                            value={amount} // Значение из состояния
                            onChange={e => setAmount(e.target.value)} // Обновляем состояние при вводе
                        />
